...
router.get("/summary", auth, allowRoles("ADMIN", "ACCOUNTANT"), async (req: Request, res: Response) => {
  try {
    ...
    if (from || to) {
      dateMatch.createdAt = {};
      if (from) dateMatch.createdAt.$gte = new Date(from as string);
      if (to) {
    ...
    const ledgerByType = ledgerStats.reduce((acc: any, item: any) => {
 ...
 * Roles: ADMIN, ACCOUNTANT
 */
router.get("/ledger", auth, allowRoles("ADMIN", "ACCOUNTANT"), async (req: Request, res: Response) => {
  try {
    const branchId = new Types.ObjectId(req.user!.branchId);
 ...
 * Roles: ADMIN, ACCOUNTANT
 */
router.post("/ledger", auth, allowRoles("ADMIN", "ACCOUNTANT"), async (req: Request, res: Response) => {
  try {
    const branchId = new Types.ObjectId(req.user!.branchId);
 ...
 * Roles: ADMIN, ACCOUNTANT
 */
router.get("/expenses/breakdown", auth, allowRoles("ADMIN", "ACCOUNTANT"), async (req: Request, res: Response) => {
  try {
    const branchId = new Types.ObjectId(req.user!.branchId);
    const { from, to } = req.query;

    const match: any = { branchId, type: "EXPENSE" };
 ...
 * Roles: ADMIN, ACCOUNTANT
 */
router.get("/taxes", auth, allowRoles("ADMIN", "ACCOUNTANT"), async (req: Request, res: Response) => {
  try {
    const branchId = new Types.ObjectId(req.user!.branchId);
