 ...
 */
export const getInventoryStats = async (branchId: Types.ObjectId) => {
  ...
  const lowStock = batches.filter(
    (b) => b.totalQuantity <= (b.product.reorderLevel || 10) && b.totalQuantity > 0
  ).length;

  const expiringSoon = batches.filter(
    (b) => b.nearestExpiry && b.nearestExpiry <= thirtyDaysFromNow && b.nearestExpiry > today
  ).length;

  const expired = batches.filter(
    (b) => b.nearestExpiry && b.nearestExpiry <= today
  ).length;
 ...
 */
export const getCategoriesWithCounts = async (branchId: Types.ObjectId) => {
 ...
 * Get expiry alerts
 */
export const getExpiryAlerts = async (branchId: Types.ObjectId) => {
  const today = new Date();
...
export const getStockReport = async (
  ...
  const reportItems = currentInventory.map((item: any) => {
    const productMovements = movements.filter(
      (m) => m.productId.toString() === item._id.toString()
    );
  ...
  // Calculate summary
  const summary = {
    totalOpeningQty: reportItems.reduce((sum: number, r: any) => sum + r.openingStock.qty, 0),
    totalOpeningValue: reportItems.reduce((sum: number, r: any) => sum + r.openingStock.amount, 0),
    totalPurchasesQty: reportItems.reduce((sum: number, r: any) => sum + r.purchases.qty, 0),
    totalPurchasesValue: reportItems.reduce((sum: number, r: any) => sum + r.purchases.amount, 0),
    totalBalanceQty: reportItems.reduce((sum: number, r: any) => sum + r.balance.qty, 0),
    totalBalanceValue: reportItems.reduce((sum: number, r: any) => sum + r.balance.amount, 0),
    totalConsumptionQty: reportItems.reduce((sum: number, r: any) => sum + r.consumption.qty, 0),
    totalConsumptionValue: reportItems.reduce((sum: number, r: any) => sum + r.consumption.amount, 0),
    totalClosingQty: reportItems.reduce((sum: number, r: any) => sum + r.closingStock.qty, 0),
    totalClosingValue: reportItems.reduce((sum: number, r: any) => sum + r.closingStock.amount, 0)
  };
 ...
 */
export const updateBatchPricing = async (
