 ...
 * Roles: ADMIN, CASHIER
 */
router.get("/:id", auth, allowRoles("ADMIN", "CASHIER"), async (req: Request, res: Response) => {
  try {
 ...
 * Roles: ADMIN, CASHIER
 */
router.post("/", auth, allowRoles("ADMIN", "CASHIER"), async (req: Request, res: Response) => {
  try {
    const branchId = getBranchId(req);
    if (!branchId) {
      return res.status(400).json({ message: "Branch not configured. Please contact admin." });
...
router.put("/:id", auth, allowRoles("ADMIN"), async (req: Request, res: Response) => {
  try {
    ...
    const customer = await Customer.findByIdAndUpdate(
      req.params.id,
      { name, phone, email, address, notes, loyaltyPoints },
      { new: true }
    );
 ...
 * Roles: ADMIN, CASHIER
 */
router.post("/:id/add-purchase", auth, allowRoles("ADMIN", "CASHIER"), async (req: Request, res: Response) => {
  try {
