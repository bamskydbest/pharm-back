 ...
 */
router.get("/stats/summary", auth, allowRoles("ADMIN"), async (req, res) => {
    try {
 ...
 * Roles: ADMIN
 */
router.get("/:id", auth, allowRoles("ADMIN"), async (req, res) => {
    try {
...
router.put("/:id", auth, allowRoles("ADMIN"), async (req, res) => {
    try {
        const { name, email, role, branchId, phone, salary, dateOfEmployment, department, address, emergencyContact, emergencyPhone, status } = req.body;
...
router.delete("/:id", auth, allowRoles("ADMIN"), async (req, res) => {
    try {
        ...
        const employee = await User.findByIdAndUpdate(req.params.id, { status: "inactive" }, { new: true }).select("-password");
        if (!employee) {
            return res.status(404).json({ message: "Employee not found" });
        }
